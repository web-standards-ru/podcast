## Эксперименты React

- [React Labs: View Transitions, Activity, and more](https://react.dev/blog/2025/04/23/react-labs-view-transitions-activity-and-more)

## Шрифты IKEA

- [IKEA’s web fonts](https://www.robinwhittleton.com/2025/04/23/ikea-web-fonts/)
- [Node.js adapter of Python font tools via Pyodide](https://github.com/web-alchemy/fonttools)

## CSS-карусели

- [CSS Carousels](https://css-tricks.com/css-carousels/)
- [Carousel Gallery](https://chrome.dev/carousel/)
- [ex-Googler](https://nerdy.dev/ex-googler)

## 10 лет SolidJS

- [A Decade of SolidJS](https://dev.to/this-is-learning/a-decade-of-solidjs-32f4)

## Как купить Chrome

- [Break Google’s Search Monopoly without Breaking the Web](https://open-web-advocacy.org/blog/break-googles-search-monopoly-without-breaking-the-web/)
- [Perplexity wants to buy Chrome if Google has to sell it](https://www.theverge.com/policy/654835/perplexity-google-antitrust-trial-remedies-chrome)
- [OpenAI wants to buy Chrome and make it an “AI-first” experience](https://arstechnica.com/ai/2025/04/chatgpt-head-tells-court-openai-is-interested-in-buying-chrome/)
- [Yahoo wants to buy Chrome](https://www.theverge.com/policy/655975/yahoo-search-web-browser-prototype-google-trial-antitrust-chrome)

## Ответы на вопросы

- [Module-Level Export Visibility: 'use internal' Directive or export internal Syntax](https://es.discourse.group/t/module-level-export-visibility-use-internal-directive-or-export-internal-syntax/2355)

[podcast@web-standards.ru](mailto:podcast@web-standards.ru)
