## Safari TP

- [Release Notes for Safari Technology Preview 168](https://webkit.org/blog/14106/release-notes-for-safari-technology-preview-168/)

## Chrome DevTools

- [What’s New in DevTools (Chrome 113)](https://developer.chrome.com/blog/new-in-devtools-113/)

## Новинки ES2023

- [What’s new in ECMAScript 2023](https://pawelgrzybek.com/whats-new-in-ecmascript-2023/)

## Экспорты в CSS-модулях

- [CSSWG Drafts: Multiple stylesheets per file](https://github.com/w3c/csswg-drafts/issues/5629)
- [Cascading Stylesheet Module scripts](https://github.com/WICG/webcomponents/blob/gh-pages/proposals/css-modules-v1-explainer.md)

## Письма без таблиц

- [Modern HTML email (tables no longer required)](https://fullystacked.net/posts/modern-html-email/)
- [Email Markup Consortium](https://emailmarkup.org/)
- [Can I email…](https://www.caniemail.com/clients/)
- [Punched card coding](https://youtu.be/xhUfiOSOk3g)

## HDR-градиенты

- [CSS HD Gradients](https://gradient.style)
- [A modern CSS gradient and color picking tool](https://twitter.com/argyleink/status/1649124742463623169)

## Node.js 20

- [Node.js 20 is now available](https://openjsf.org/blog/2023/04/18/node-js-20-now-available/)

## Зачем Deno?

- [Deno vs. Node: No One is Ready for the Move](https://cult.honeypot.io/reads/deno-vs-node-main-differences/)

## Ответы на вопросы

- [Придумал вариант применения @layer](https://github.com/calmosowl/layers-test)
- [CSS and JavaScript as first-class citizens in Eleventy](https://pepelsbey.dev/articles/eleventy-css-js/)
- [Свежий взгляд на Gulp: функции и ES-модули](https://www.youtube.com/watch?v=fgJmCevEtL4)

[podcast@web-standards.ru](mailto:podcast@web-standards.ru)
