## Новости Safari

- [WebKit Features in Safari 16.5](https://webkit.org/blog/14154/webkit-features-in-safari-16-5/)
- [Release Notes for Safari Technology Preview 170](https://webkit.org/blog/14167/release-notes-for-safari-technology-preview-170/)

## Снова про Baseline

- [On browser compatibility and support baselines](https://molily.de/browser-compatibility-baseline/)
- [Дока: grid](https://doka.guide/css/grid/)

## Firefox 113

- [Firefox Developer Experience](https://fxdx.dev/)
- [Firefox 113](https://tanalin.com/blog/posts/firefox-113/)

## TypeScript 5.1 RC

- [Announcing TypeScript 5.1 RC](https://devblogs.microsoft.com/typescript/announcing-typescript-5-1-rc/)
- [Introducing Deopt Explorer](https://devblogs.microsoft.com/typescript/introducing-deopt-explorer/)

## Семантика popover

- [Semantics and the popover attribute: what to use when?](https://hidde.blog/popover-semantics/)
- [Попап вместо селекта](https://youtu.be/mG0VFWCTnZI)

## Перфоманс DOM

- [How large DOM sizes affect interactivity, and what you can do about it](https://web.dev/dom-size-and-interactivity/)
- [The truth about CSS selector performance](https://blogs.windows.com/msedgedev/2023/01/17/the-truth-about-css-selector-performance/)

## Ответы на вопросы

[podcast@web-standards.ru](mailto:podcast@web-standards.ru)
