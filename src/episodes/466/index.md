## Относительные цвета

- [CSS Relative Colors](https://ishadeed.com/article/css-relative-colors/)

## TypeScript на Go

- [A 10x Faster TypeScript](https://devblogs.microsoft.com/typescript/typescript-native-port/)
- [TypeScript Go Playground](https://tsgo.sxzz.dev/)
- [I’m porting tsc to Go](https://kdy1.dev/2022-1-26-porting-tsc-to-go#heading-why-not-rust)

## UI для Git в Zed

- [Native Git support in Zed](https://zed.dev/blog/git)

## Baseline для CSS в ESLint

- [Support for CSS and Baseline has shipped in ESLint](https://web.dev/blog/eslint-baseline-integration)

## Усталость от JS

- [JavaScript Fatigue Strikes Back](https://allenpike.com/2025/javascript-fatigue-ssr)

## Ответы на вопросы

- [Making a custom clear text field button](https://www.scottohara.me/blog/2022/02/19/custom-clear-buttons.html)

[podcast@web-standards.ru](mailto:podcast@web-standards.ru)
