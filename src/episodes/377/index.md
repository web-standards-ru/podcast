## Поддержите нас

- [Патреон](https://www.patreon.com/webstandards_ru)
- [Бусти](https://boosty.to/webstandards_ru)

## Firefox 115

- [Firefox 115 for developers](https://developer.mozilla.org/en-US/docs/Mozilla/Firefox/Releases/115)

## Юникод и эмоджи

- [Intent to Prototype: CSS font-variant-emoji](https://groups.google.com/a/chromium.org/g/blink-dev/c/MaXgbE4vTbk/m/Q3QbI37IBQAJ)
- [Emoji under the hood](https://tonsky.me/blog/emoji/)

## Контейнеры вместо медиа

- [Can We Query the Root Container?](https://www.oddbird.net/2023/07/05/contain-root/)

## DOM­Content­Loaded

- [In Defence of DOM­Content­Loaded](https://csswizardry.com/2023/07/in-defence-of-domcontentloaded/)

## Избавляемся от SVG-in-JS

- [Breaking Up with SVG-in-JS in 2023](https://kurtextrem.de/posts/svg-in-js/)

## Бесполезные слэши в тегах

- [The case against self-closing tags in HTML](https://jakearchibald.com/2023/against-self-closing-tags-in-html/)

## Ответы на вопросы

- [HTMLDialogElement: cancel event](https://developer.mozilla.org/en-US/docs/Web/API/HTMLDialogElement#events)

[podcast@web-standards.ru](mailto:podcast@web-standards.ru)
